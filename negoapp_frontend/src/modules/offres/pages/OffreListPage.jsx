...
export default function OffreListPage() {
  ...
  return <div>
    ...
    <div className="bg-white rounded-lg border border-slate-200 overflow-hidden"><table className="w-full text-sm"><thead className="bg-slate-50 text-slate-500 text-xs uppercase"><tr><th className="text-left px-4 py-3">Article</th><th className="text-left px-4 py-3">Fournisseur</th><th className="text-right px-4 py-3">Prix</th><th className="text-right px-4 py-3">PVA</th><th className="text-right px-4 py-3">MOQ</th><th className="text-right px-4 py-3">Lead time</th><th className="text-left px-4 py-3">Validité</th></tr></thead><tbody>{data.results.map(o => <tr key={o.id} className="border-t border-slate-100 hover:bg-slate-50"><td className="px-4 py-3"><Link className="font-mono text-teal-700 font-medium" to={`/offres/${o.id}/modifier`}>{o.article_cpn}</Link><div className="text-xs text-slate-500">{o.article_desc}</div></td><td className="px-4 py-3">{o.fournisseur_nom}</td><td className="px-4 py-3 text-right font-mono">{Number(o.prix_unitaire).toFixed(2)} {o.devise}</td><td className="px-4 py-3 text-right font-mono">{Number(o.pva).toFixed(2)} {o.devise}</td><td className="px-4 py-3 text-right">{o.moq}</td><td className="px-4 py-3 text-right">{o.lead_time} j</td><td className="px-4 py-3">{o.date_validite || '—'}</td></tr>)}{data.results.length === 0 && <tr><td colSpan="7" className="text-center py-10 text-slate-400">Aucune offre enregistrée</td></tr>}</tbody></table></div>
  </div>;
}
